...
def add_fruit():
    try:
        ...
        if (name=="11"):
            ...
            cnt = db.save_bmi_record_name(input_name)
            if int(cnt[0]["cnt"]) == 0:
                success = db.insert_bmi_record_name(input_name)
                last_code = db.get_last_fruits()
            else:
                last_code = db.get_fruits(input_name)
                
            if last_code[0]["id"] != "":
                success = db.insert_bmi_record(last_code[0]["id"],age_name,gender_name)
        else: 
