...
class Database:
    ...
    def save_bmi_record_name(self,input_name):
        """최근 과일정보 기록을 가져옵니다"""
        try:
            if self.connection is None:
                print("데이터베이스 연결이 없습니다.")
    ...
    def insert_bmi_record_name(self,input_name):
        """과일을 데이터베이스에 저장"""
        try:
            if self.connection is None:
                print("데이터베이스 연결이 없습니다.")
            ...
            with self.connection.cursor() as cursor:
                query = """
                INSERT INTO fruits_code(fruit_nm)
                VALUES((%s))
                """
                cursor.execute(query, (input_name))
    ...
    def get_fruits(self,input_name):
        """과일을 데이터베이스에 저장"""
        try:
            if self.connection is None:
                print("데이터베이스 연결이 없습니다.")
